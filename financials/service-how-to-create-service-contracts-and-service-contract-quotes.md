...
1. Choisissez l'icône ![Page ou rapport pour la recherche](media/ui-search/search_small.png "icône Page ou rapport pour la recherche"), entrez **Contrats de service** ou **Devis contrat de service**, puis sélectionnez le lien associé.  
...
Une fois les étapes suivantes effectuées, un contrat de service est créé avec l'état **Signé**. Si une facture service est crée pour la période de début du contrat, le montant facturé est calculé comme suit, selon que le montant est détaillé ou non.  
...
4. Si la date de début du contrat est antérieure au début de la période de facturation suivante, vous serez invité à créer une facture pour la période de démarrage du contrat. Choisissez **Oui**.  
 ...
 La facture service est reportée sur le compte service du contrat, même si le contrat est prépayé. 
...
4. Dans le champ **N° client**, entrez le numéro du client du contrat de service.  
    ...
    > [!NOTE]  
    >  Vous pouvez comparer l'avoir avec le document validé initialement dans la fenêtre **Factures service enreg.**. Choisissez l'icône ![Page ou rapport pour la recherche](media/ui-search/search_small.png "icône Page ou rapport pour la recherche"), entrez **Factures de service reportées**, puis sélectionnez le lien associé.  
...
>   
>  Si le contrat est détaillé et prépayé, et que vous avez reporté des factures pour le contrat, vous pouvez créer une note de crédit pour ce contrat. Sous l'onglet de **Actions** , dans le groupe **Fonctions**, choisissez **Créer avoir**. Cela n'est pas nécessaire si la case à cocher **Avoirs automatiques** sur le raccourci **Détails facture** est sélectionnée. Dans ce cas une note de crédit est créée automatiquement lorsque vous supprimez une ligne contrat. 
...
| Options Coût ligne | Description|
|----------------------------------|---------------------------------------|  
...
| Options Valeur ligne | Description|
|----------------------------------|---------------------------------------|  
...
1. Choisissez l'icône ![Page ou rapport pour la recherche](media/ui-search/search_small.png "icône Page ou rapport pour la recherche"), entrez **Devis de contrat de service** et sélectionnez le lien associé.  
2. Choisissez le devis auquel ajouter des escomptes.  
3. Choisissez l'action **Escomptes service**. La fenêtre **Escomptes contrat/service** s'affiche.  
...
> [!Tip]  
>  Pour ajouter des escomptes contrat directement à un contrat service, exécutez la procédure décrite dans la fenêtre **Contrat de service**.  
...
> [!NOTE]  
>  Dans ce cas, seuls les contrats non annulés sont pris en compte. L'état des devis contrat n'est pas pris en compte.  
...
1. Choisissez l'icône ![Page ou rapport pour la recherche](media/ui-search/search_small.png "icône Page ou rapport pour la recherche"), entrez **Contrats de service**, puis sélectionnez le lien associé. Ouvrez le contrat de service dont vous voulez modifier le propriétaire.  
...
6. Dans le champ **Nouveau code de livraison**, choisissez l'adresse.  
7. Choisissez **OK** pour modifier le code de livraison et le client des contrats de service.  
...
1. Choisissez l'icône ![Page ou rapport pour la recherche](media/ui-search/search_small.png "icône Page ou rapport pour la recherche"), entrez **Réviser tarifs de contrat de service**, puis sélectionnez le lien associé. 
...
## <a name="to-post-prepaid-contract-entries"></a>Pour reporter des écritures contrat prépayé  
Si vous utilisez des contrats de service prépayés, vous devez reporter régulièrement les écritures contrat prépayé, de manière à transférer les paiements anticipés des comptes contrat prépayé sur les comptes contrats ordinaires.  
  
Avant de pouvoir reporter des écritures contrat prépayées, vous devez spécifier une série de numéros dans le champ **N° doc. report prépayé** de la fenêtre **Configuration de la gestion des services**.  

1. Choisissez l'icône ![Page ou rapport pour la recherche](media/ui-search/search_small.png "icône Page ou rapport pour la recherche"), entrez **Reporter les écritures de contrat prépayées**, puis sélectionnez le lien associé.  
...
6. Choisissez **OK** pour reporter les écritures.

## <a name="changing-the-service-contract-status"></a>Modification de l'état d'un contrat de service
...
4. Dans le champ **Code motif annulation**, choisissez le code motif adéquat. Pour ajouter des codes motif, cliquez sur l'action **Avancé**.  
     ...
     Si le champ **Utiliser motif annulation contrat** est coché dans la fenêtre **Configuration de la gestion des services**, vous devez indiquer un code motif annulation lorsque vous annulez des contrats.  
  
5. Dans le champ **État**, choisissez **Annulé**.  
