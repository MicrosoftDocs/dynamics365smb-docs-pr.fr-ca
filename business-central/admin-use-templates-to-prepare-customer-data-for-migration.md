...
description: Si vous utilisez RapidStart Services, vous pouvez utiliser des modèles de configuration pour structurer les données client existantes avant de migrer les données principales vers la nouvelle compagnie dans Business Central.
author: SorenGP
ms.topic: conceptual
ms.devlang: na
ms.tgt_pltfrm: na
...
Vous pouvez créer un nouveau modèle de données si les modèles par défaut ne répondent pas aux besoins de votre compagnie.  
...
2. Sélectionnez l'action **Nouveau**.
3. Complétez le champ **Code**.
4. Dans le champ **Code table**, entrez la table à laquelle ce modèle s'applique. Le champ **Nom table** est automatiquement renseigné lorsque le champ **Code table** est défini.
