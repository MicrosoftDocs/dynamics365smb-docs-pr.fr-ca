...
Conversation instantanée avec Copilot, également appelé chat, est une fonctionnalité interactive qui répond aux questions sans que les utilisateurs aient à naviguer dans l’interface utilisateur ou dans la documentation du produit. Le volet Copilot est disponible partout dans le [!INCLUDE[prod_short](includes/prod_short.md)] client.
...
- Copilot convertit l’entrée en langage naturel en une requête composée de critères de recherche, de tri et de filtrage dans une table.
  ...
  La capacité utilise les capacités natives de recherche de données de [!INCLUDE[prod_short](includes/prod_short.md)] pour trouver des données correspondantes à partir de tables au sein de la base de données des compagnies. La recherche s’exécute sous votre propre identité pour des raisons de sécurité et de conformité. Il ne recherche pas en dehors du [!INCLUDE[prod_short](includes/prod_short.md)] base de données.
...
Vous avez la possibilité de fournir des commentaires sur chaque réponse Copilot et de signaler tout contenu inexact ou inapproprié pour aider Microsoft à améliorer cette fonctionnalité. 
...
- Nous analysons et utilisons vos commentaires sur la fonctionnalité pour nous aider à améliorer les réponses.
...
- Limitations générales de l'IA
  ...
  Les systèmes d’IA sont des outils précieux, mais ils ne sont pas déterministes. Le contenu qu’ils génèrent peut ne pas être exact. Il est donc important d’utiliser votre jugement pour examiner et vérifier les réponses de Copilot avant de prendre des décisions qui pourraient affecter les parties prenantes telles que les clients et les partenaires. Pour la plupart des réponses, Copilot inclut également des citations ou des liens de référence que vous pourrez utiliser pour vérifier rapidement si Copilot a trouvé la bonne réponse. Par exemple, lorsqu’on lui demande comment effectuer une tâche, Copilot inclut des liens vers l’article source. Lorsqu’on lui demande de rechercher un enregistrement en fonction de critères spécifiques, Copilot inclut des liens qui décrivent la page de liste qu’il a identifiée comme sujet de conversation. Il fournit également des informations sur les filtres ou tris appliqués pour obtenir une réponse.
...
- Limites linguistiques
  ...
  - La qualité des réponses peut être moindre dans les conditions suivantes :
    ...
    - Le paramètre de langue de l’utilisateur dans [!INCLUDE[prod_short](includes/prod_short.md)] diffère de la langue principale des données dans la [!INCLUDE[prod_short](includes/prod_short.md)] base de données.
