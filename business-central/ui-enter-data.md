...
Lorsque vous entrez des données sur les pages, certains champs sont marqués par un astérisque rouge. L'astérisque rouge signifie que le champ doit être rempli pour effectuer un certain processus. Par exemple, lorsque vous reportez une transaction qui utilise la valeur dans le champ.  

Même si un champ est obligatoire, vous n'êtes pas obligé de le remplir avant de passer à d'autres champs ou de fermer la page. L'astérisque rouge sert uniquement à rappeler que la fin d'un certain processus restera bloquée.  
...
Vous pouvez copier une ou plusieurs lignes d'une liste ou d'un seul champ sur une page. Collez ensuite ce que vous avez copié dans la même page, une autre page ou un document externe. Vous pourriez, par exemple, coller dans Microsoft Excel ou la messagerie Outlook. En raccourci, pour copier, appuyez sur CTRL+C (cmd+C dans macOS) sur votre clavier. Pour coller, appuyez sur CTRL+V ou cmd+V dans macOS.
...
Lors de la saisie de nombres dans les champs de quantité, tels que le champ **Quantité** d'une ligne journal article, vous pouvez entrer la formule plutôt que la somme des quantités.  
...
|f|Spécifie que la date après c est une date de fermeture, par exemple C123101.|  
...
Vous entrez une durée sous la forme d'un nombre suivi d'une unité de mesure.  
...
|2j 6h 30m 56s 600ms|2 jours 6 h 30 m 56 s 600 ms|  
 ...
 Pour connaître l'unité de mesure utilisée pour un champ de durée, saisissez un nombre et observez dans quelle unité de mesure il est converti.  
