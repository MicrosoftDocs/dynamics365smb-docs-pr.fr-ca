...
La saisie rapide est une fonction conçue pour saisir les données avec le clavier. La fonction Saisie rapide fonctionne sur les champs (comme les pages Fiche) et dans les listes (lignes et colonnes). Cela est utile lors de l'exécution de tâches de saisie répétitives qui exigent de créer plusieurs enregistrements dans la séquence, comme un lot de documents de vente ou l'enregistrement de nouveaux articles.
...
Lorsque vous entrez des données sur les pages, certains champs sont marqués par un astérisque rouge. L'astérisque rouge signifie que le champ doit être renseigné pour terminer un processus qui utilise ce champ, par exemple, reporter une transaction qui utilise la valeur du champ.  
...
|p|Cela précise une période comptable, où p signifie la première période comptable, p2 la deuxième période comptable, etc. |
...
|f|Cela spécifie que la date après f est une date de fermeture, par exemple F123101.|  
