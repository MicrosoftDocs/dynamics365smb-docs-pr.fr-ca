---
title: 'Faire, ajuster et reclasser l’inventaire'
...
Pour modifier les attributs des écritures article, vous pouvez utiliser un journal reclassement article. Les attributs typiques à reclasser incluent les dimensions et les codes de promotion de vente. Vous pouvez aussi utiliser les journaux reclassement pour les transferts en reclassant les codes de zone et d'emplacement. Des étapes spéciales s'appliquent lorsque vous souhaitez reclasser les numéros de série ou de lot et leurs dates d'expiration. Pour plus d'informations, voir [Utiliser les numéros de lot et de série](inventory-how-work-item-tracking.md).
...
Pour vérifier si la quantité enregistrée est la même que la quantité physique en stock, comptabiliser votre inventaire physique. Autrement dit, comptez les articles réellement disponibles. En règle générale, ces comptages ont lieu à la fin d’un exercice financier, mais certaines entreprises comptent les articles plus souvent. S’il existe des différences, reportez les quantités réelles dans les comptes article avant de procéder à l’évaluation de l'inventaire.
...
- Imprimez le rapport que vous souhaitez utiliser pour le décompte.
...
### <a name="to-calculate-expected-inventory-in-basic-warehouse-configurations"></a>Pour calculer l’inventaire prévu dans les configurations d’entrepôt de base
...
2. Choisissez l’action  **Calculer l’inventaire** dans **Préparer**.
...
1. Sur la page **Journaux d’inventaire physique** contenant l’inventaire prévu calculé, choisissez l’action **Imprimer** depuis **Accueil**.
2. Sur la page **Liste d’inventaire physique**, spécifiez si le rapport affichera la quantité calculée et les articles d’inventaire par numéros de série et de lot.
...
1. Sur chaque ligne de la page **Journaux d’inventaire physique** où l’inventaire réel disponible, tel que déterminé par le décompte physique, diffère de la quantité calculée, saisissez l’inventaire réel disponible dans le champ **Qté (inventaire physique)** .
...
2. Pour Ajuster les quantités calculées aux quantités réellement comptées, choisissez l’action **Publier** depuis **Accueil**.

    Le report crée les écritures article et les écritures inventaire physique. Ouvrez la page Fiche article de l’article pour rechercher ses écritures inventaire physique.
...
4. Pour vérifier le nombre, ouvrez la page  **Article n°carte** de l’article et choisissez l’action  **Écritures du grand livre d’inventaire physique** depuis **Article**
1. .
...
Synchroniser le grand livre article et l’entrepôt <!--warehouse what?--> avant de compter l’inventaire physique. Sinon, ce que vous reportez dans le journal inventaire physique et le grand livre article est le résultat de l’inventaire physique combiné à d’autres ajustements d’entrepôt pour les articles. Pour en savoir plus, voir [Synchroniser les quantités dans le grand livre article et l'entrepôt](inventory-how-count-adjust-reclassify.md#to-synchronize-the-adjusted-warehouse-entries-with-the-related-item-ledger-entries).
...
3. Définissez les filtres pour spécifier les articles à compter dans le journal, puis choisissez **OK**.
   ...
   Si vous ne comptez qu’un article dans certaines zones mais pas dans d’autres, vous pouvez saisir les différences et les reporter ultérieurement dans la page **Journal article** en utilisant l’action **Calculer ajustement entrepôt**.  
...
2. Après avoir saisi toutes les quantités réelles, choisissez l’action **Enregistrer**.  
    ...
    - Si les quantités calculées et les quantités réelles sont différentes, une quantité négative ou positive est enregistrée pour la zone, et une quantité d’équilibre est reportée dans la zone d’ajustement de l'emplacement.  
...
Lorsque vous reportez un inventaire physique, vous ne le reportez pas dans les grands livres article, inventaire physique ou valeur. Cependant, les enregistrements sont disponibles pour le rapprochement en cas de besoin. Pour que les quantités restent exactes, après avoir compté les articles dans toutes les zones, reportez les résultats.<!--physical inventory journal--> Pour en savoir plus, voir [Synchroniser les quantités dans le grand livre article et l'entrepôt](inventory-how-count-adjust-reclassify.md#to-synchronize-the-adjusted-warehouse-entries-with-the-related-item-ledger-entries).

## <a name="to-do-cycle-counting"></a>Pour effectuer un inventaire tournant

Vous pouvez compter les articles aussi souvent que vous le souhaitez. Par exemple, parce qu’ils ont plus de valeur ou parce qu’ils sont les plus vendus. Spécifiez la fréquence de comptage en attribuant des périodes de comptage spéciales aux articles.
...
Sur la page **Journaux d’articles**, vous pouvez publier des transactions d’articles directement dans l’inventaire Ajuster pour les achats, les ventes et les modifications positives ou négatives sans utiliser de documents.
...
5. Dans le champ **Quantité**, si vous trouvé des articles supplémentaires, entrez une quantité positive. Si des articles sont manquants, saisissez une quantité négative.  
...
Reportez les enregistrements de zone d’ajustement dans le grand livre article pour les périodes définies. Certaines compagnies reportent des ajustements quotidiens dans le grand livre article, tandis que d’autres effectuent le rapprochement moins souvent.
...
4. Sur le raccourci **Options**, entrez manuellement un numéro dans le champ **N° document**. Le numéro s'affiche sur les lignes journal article.
...
6. Reportez les lignes journal pour entrer les différences de quantité dans le grand livre article. Les quantités dans les zones et le registre des articles correspondent désormais.  
