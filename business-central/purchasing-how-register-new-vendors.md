...
ms.author: bholtorf
---
# <a name="register-new-vendors"></a><a name="register-new-vendors"></a><a name="register-new-vendors"></a>Enregistrer de nouveaux fournisseurs
...
> [!TIP]  
> Si vous ne connaissez pas l’adresse de facturation qui sera utilisée pour chaque facture fournisseur, ne renseignez pas le champ **N°**. du raccourci **Général**. Sélectionnez plutôt le numéro du fournisseur créancier après avoir configuré un devis, une commande ou un en-tête facture.
...
Vous pouvez modifier les informations des fiches fournisseur à tout moment. Toutefois, si vous avez reporté une transaction pour un fournisseur, vous ne pouvez pas supprimer la fiche, car les écritures peuvent être nécessaires aux fins d’audit. Pour supprimer des fiches fournisseur avec des écritures, contactez votre partenaire Microsoft pour le faire via le code.
...
3. Pour réutiliser les axes analytiques dans les modèles, sélectionnez l'action **Axes analytiques**. La page **Modèles dimension** s’ouvre et affiche tous les codes dimension qui sont définis pour le fournisseur.
