...
Le coût moyen d’un article est calculé avec une moyenne pondérée périodique. La moyenne est basée sur la période de coût moyen spécifié dans [!INCLUDE[prod_short](includes/prod_short.md)].  
...
|Champ|Description|  
|---------------------------------|---------------------------------------|  
|**Période de coût moyen**|Spécifie à quel moment le coût moyen est calculé. Les options possibles sont les suivantes :<br /><br /> - **Jour**<br />- **Semaine**<br />- **Mois**<br />- **Période comptable**<br /><br /> Le coût moyen calculé pour cette période est affecté aux diminutions d'inventaire reportées au cours de la période coût moyen.|  
|**Type calcul coût moyen**|Indique le mode de calcul du coût moyen. Les options possibles sont les suivantes :<br /><br /> - **Article**<br />- **Article, variante et magasin**<br /> Avec cette option, le coût moyen est calculé pour chaque article, pour chaque emplacement et pour chaque variante de l'article. Le coût moyen de cet article dépend du lieu de stockage et de la variante de l’article que vous avez sélectionnés (par exemple, la couleur).|  
...
| **Date de validation** | **Type d'écriture gr. livre art.** | **Quantité** | **Coût indiqué (réel)** | **N° séquence** |
|--|--|--|--|--|
...
[Processus métier pour le calcul des coûts des produits et comment il se rapporte à d’autres processus avec Dynamics 365](/dynamics365/guidance/business-processes/design-to-retire-define-product-costing-overview)  
