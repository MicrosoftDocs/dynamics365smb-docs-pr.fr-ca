...
> [!NOTE]
> Cette procédure décrit comment effectuer un inventaire à l'aide de documents, une méthode qui offre un plus grand contrôle et prend en charge la répartition de l'inventaire vers plusieurs employés. Vous pouvez également effectuer la tâche à l'aide de journaux, les pages **Journaux inventaire** et **Journaux inventaire entrepôt**. Pour plus d'informations, voir [Inventaire, ajustement et reclassement de l'inventaire avec les journaux](inventory-how-count-adjust-reclassify.md).<br /><br />
> Notez que si vous utilisez la fonctionnalité Zones ou Zones, vous ne pouvez pas utiliser les commandes d'inventaire physique. À la place, utilisez la page **Journal inventaire entrepôt** pour faire l'inventaire de vos écritures entrepôt avant de les synchroniser avec les écritures article.
...
Vous pouvez également choisir l'action **Diff. commande inventaire physique** pour visualiser les différences entre la quantité prévue et la quantité comptabilisée.
