...
Par exemple, si la compagnie actuelle se trouve dans un environnement qui est une copie d’un autre environnement, toutes les tâches programmées sont automatiquement arrêtées. Utilisez la page **Tâches programmées** pour définir les tâches comme prêtes à être exécutées dans la file d'attente des travaux.  
