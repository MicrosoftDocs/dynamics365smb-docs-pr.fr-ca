...
Le rôle de vendeur est souvent considéré comme tourné vers l'extérieur dans une entreprise. Toutefois, il peut être utile pour les vendeurs d'être en mesure de regarder à l'intérieur de l'entreprise et d'observer ce qu'il s'y passe en arrière-plan. En intégrant [!INCLUDE[prod_short](includes/prod_short.md)] et [!INCLUDE[crm_md](includes/crm_md.md)], vous pouvez donner cet aperçu à vos commerciaux. L’intégration permet aux utilisateurs de consulter les informations [!INCLUDE[prod_short](includes/prod_short.md)] pendant qu’ils travaillent [!INCLUDE[crm_md](includes/crm_md.md)]. Par exemple, dans le cadre de la préparation d'un devis, il peut s'avérer utile de savoir si vous avez suffisamment d'articles dans l'inventaire pour répondre à la commande. Pour plus d’informations, voir [Utiliser Dynamics 365 Sales depuis Business Central](marketing-integrate-dynamicscrm.md).

> [!NOTE]
> Cet article décrit le processus d’intégration des versions en ligne de [!INCLUDE[crm_md](includes/crm_md.md)] et [!INCLUDE[prod_short](includes/prod_short.md)] through [!INCLUDE[prod_short](includes/cds_long_md.md)]. Pour plus d'informations sur la configuration sur site, voir [Préparation de l'intégration à Dynamics 365 Sales On-Premises](/dynamics365/business-central/dev-itpro/administration/prepare-dynamics-365-for-sales-for-integration).
...
> [!TIP]
> Si vous souhaitez également intégrer Microsoft Dynamics 365 Field Service, guide fournit un étape facultatif qui peut vous aider. Pour en savoir plus sur l’intégration à Field Service, accédez à [Intégrer avec Microsoft Dynamics 365 Field Service](admin-integrate-field-service.md).
...
Lorsque vous configurez votre intégration, certaines options contrôlent la direction dans laquelle vous synchronisez les commandes client et la manière dont vous les soumettez.
...
2. Étant donné que les nouveaux produits sont créés dans [!INCLUDE[crm_md](includes/crm_md.md)] avec de nouveaux groupes d’unités, pour éviter les noms en double, effectuez l’une des étapes suivantes :
...
|Articles|Lorsqu’il synchronise les articles avec [!INCLUDE[crm_md](includes/crm_md.md)] produits, [!INCLUDE[prod_short](includes/prod_short.md)] crée automatiquement une liste de prix dans [!INCLUDE[crm_md](includes/crm_md.md)]. Pour éviter les erreurs de synchronisation, vous ne devez pas modifier cette liste de prix manuellement.|
...
|Commandes vente|Lorsque vous activez l’intégration des commandes client, lorsque vous validez les commandes client dans [!INCLUDE[prod_short](includes/prod_short.md)] qui ont été créées à partir des commandes client soumises dans [!INCLUDE[crm_md](includes/crm_md.md)] elles se synchronisent avec les commandes client dans [!INCLUDE[crm_md](includes/crm_md.md)]. Avant de synchroniser les commandes, nous vous recommandons de synchroniser d’abord toutes les tables impliquées dans la commande. Par exemple, les vendeurs et les listes de prix. Le champ Code représentant de l’en-tête de commande définit le propriétaire de la table couplée dans [!INCLUDE[crm_md](includes/crm_md.md)].|
...
L’ Microsoft Power Platform équipe [a annoncé](/power-platform/important-changes-coming#deprecation-of-office365-authentication-type-and-organizationserviceproxy-class-for-connecting-to-dataverse) qu’elle déprécie le Office 365 type d’authentification. Si vous utilisez une version de [!INCLUDE[prod_short](includes/prod_short.md)] sur site antérieure à [!INCLUDE [prod_short](includes/prod_short.md)] 2019 vague de lancement 1, vous devez utiliser le type d’authentification OAuth pour Connecter pour [!INCLUDE[crm_md](includes/crm_md.md)] en ligne. Les étapes de cette section décrivent comment établir la connexion aux versions de produit suivantes :
...
* Vous devez avoir un abonnement Microsoft Azure. Un compte d’essai fonctionne pour l’enregistrement des applications.
