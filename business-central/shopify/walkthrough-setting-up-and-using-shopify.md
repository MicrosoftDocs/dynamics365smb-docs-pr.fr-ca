...
4. Sélectionnez *Solde disponible projeté à aujourd’hui* dans le **Calcul du stock** pour activer la synchronisation de l'inventaire pour un emplacement Shopify sélectionné.
...
7. Sélectionnez **OK** et attendez que la synchronisation initiale des articles, des prix, des images et de l'inventaire soit terminée.
...
Dans **Admin Shopify**, notez que la compagnie et le client ont tous deux été importés. Ouvrez les clients et prenez note du récapitulatif de la compagnie avec un lien vers la compagnie, l’emplacement et les autorisations attribuées. Sélectionnez **[...]** dans le récapitulatif **Compagnie**, puis sélectionnez **Envoyer un courriel d’accès B2B** pour inviter le client.
...
12. Sélectionnez **Ajuster l'inventaire** et dans le champ **Nouvel inventaire**, saisissez `100` pour les emplacements *EAST* et *WEST*. 
13. Sélectionnez **OK**.
...
4. Sélectionnez *N° d’article + code de variante* dans le champ **Mappage unité de stock**.
5. Sélectionnez *Continuer* dans le champ **Règle d'inventaire par défaut** .
6. Sélectionnez *Brouillon* dans le champ **État des produits créés**.
7. Sélectionnez *État à archiver* dans le champ **Action pour le produit supprimé**.
8. Sélectionnez *SHOPIFY* dans le champ **Groupe de prix client**.
9. Sélectionnez *VENTE AU DÉTAIL* dans le champ **Groupe escompte client**.
...
8. Sélectionnez **OK** et attendez que la synchronisation initiale des articles, des prix, des images et de l'inventaire soit terminée.
...
* *Table de conférence ANTWERP* est *Active*, car nous avons modifié son état dans la fenêtre **Produit Shopify**.
...
Notez que l'inventaire de la table de conférence ANTWERP est de 100, car nous avons configuré le système pour utiliser l’inventaire uniquement à partir de deux emplacements : MAIN et EAST. L'inventaire à un autre emplacement (WEST) est ignoré.
...
* Modifiez l'état de *Bureau Athens* sur *Actif* et sélectionnez **Aperçu**.

Dans la **boutique en ligne Shopify**, ouvrez le catalogue de produits et recherchez le produit *Bureau ATHENS*. Notez que différentes options sont disponibles. Pour différentes options, les prix sont différents. Faites attention aux informations d'escompte.
...
5. Sélectionnez **Clients**, puis **Compagnies** dans la barre latérale de **Admin Shopify**.
...
10. Sélectionnez *GRAND CPTE* dans le champ **Groupe escompte client**.
...
Dans la **boutique en ligne Shopify**, ouvrez le catalogue de produits et recherchez le produit *Bureau ATHENS*. Notez que les prix sont des informations d'escompte.

## Procédure pas à pas : vérification et synchronisation des commandes pour l’acheteur individuel et le représentant de la compagnie
...
2. Connectez-vous à l’aide d’un code de vérification unique à 6 chiffres envoyé par courriel que vous saisissez.
...
4. Choisissez **[...]** dans le récapitulatif **École des Beaux-Arts**, puis **Modifier les modalités de paiement** et sélectionnez *Dû à l’exécution*.
5. Choisissez **[...]** dans le récapitulatif **Clients**, puis **Ajouter un client** et ajoutez-en un avec le courriel que vous avez utilisé pour vous connecter à la boutique plus tôt.
6. Dans la **boutique en ligne Shopify**. Choisissez icône **compte**. Entrer le courriel auquel vous avez accès.
7. Connectez-vous à l’aide d’un code de vérification unique à 6 chiffres envoyé par courriel que vous saisissez.
...
10. Notez que les champs Compte, Destinataire et Mode de paiement sont renseignés.
...
3. Dans la commande soumise au nom de la compagnie, vous pouvez voir la valeur dans le champ **Numéro de bon de commande**, qui est également transférée dans le champ **Numéro de document externe** du document vente créé.
4. Étant donné que nous avons configuré la compagnie B2B pour gérer les paiements en dehors de Shopify, l'**état financier** est réglé sur *En attente*. Une fois le paiement reçu, sélectionnez le **Marquer comme payé** action. La situation financière sera mise à jour dans Shopify. 

## Procédure pas à pas : importer des articles, des clients, des compagnies depuis Shopify
...
10. Sélectionnez *Tous les clients* dans **Importation de compagnie depuis Shopify**.
11. Activez le bouton à bascule **Créer automatiquement des compagnies inconnues**.
...
* Les compagnies Shopify sont importées. Pour vérifier, sélectionnez l’icône ![Ampoule qui ouvre la fenêtre de recherche](../media/ui-search/search_small.png "Dites-moi ce que vous voulez faire") , saisissez **Compagnies Shopify** et sélectionnez le lien associé.
