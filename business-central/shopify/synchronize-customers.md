...
3. Dans certains cas, le **Code modèle client** défini par pays ne suffit pas pour assurer le calcul correct des taxes. Par exemple, pour les pays avec taxe de vente. Dans ce cas, les **régions fiscales** peuvent constituer un complément utile.
...
|Champ|Désignation|
|------|-----------|
|**Exporter les clients dans Shopify**|Sélectionnez si vous prévoyez d’exporter tous les clients avec une adresse de courriel valide à partir de [!INCLUDE[prod_short](../includes/prod_short.md)] dans Shopify en bloc, soit manuellement en utilisant l’action **Synchroniser les clients**, soit via la file d’attente des travaux pour les mises à jour récurrentes.|
