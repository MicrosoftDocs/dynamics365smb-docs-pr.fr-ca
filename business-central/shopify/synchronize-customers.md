...
5. Le champ **Région fiscale** contient également une paire **Code pays** et **Nom du comté**. Cette paire est utile lorsque le connecteur doit convertir un code en un nom, ou vice versa.
...
Pour les adresses où le comté/la province est utilisé(e), sélectionnez **Code** ou **Nom** dans le champ **Source comté** sur la page **Fiche magasin Shopify**. Ceci spécifie le type de données stockées dans [!INCLUDE[prod_short](../includes/prod_short.md)] dans le champ **Comté**. N’oubliez pas d’initialiser les modèles de clients par pays afin que le mappage code/nom du comté soit prêt. 
