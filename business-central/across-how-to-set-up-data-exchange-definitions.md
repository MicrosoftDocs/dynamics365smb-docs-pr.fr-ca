...
3. Sur le raccourci **Général**, décrivez la définition d'échange de données et le type de fichier de données en renseignant les champs comme indiqué dans le tableau suivant.  
    ...
    |**Étiquette d'en-tête**|Si une ligne d'en-tête existe à plusieurs endroits dans le fichier, saisissez le texte de la première colonne sur la ligne d'en-tête.<br /><br /> Ceci garantit que les données d'en-tête ne sont pas importées. **Remarque** : ce champ ne s'applique qu'aux importations.|  
    |**Étiquette de pied de page**|Si une ligne de pied de page existe à plusieurs endroits dans le fichier, saisissez le texte de la première colonne sur la ligne de pied de page.<br /><br /> Ceci garantit que les données de pied de page ne sont pas importées. **Remarque** : ce champ ne s'applique qu'aux importations.|  
...
8. Dans le raccourci **Définitions des colonnes**, modifiez ou renseignez les champs comme indiqué dans le tableau ci-dessous.  
    ...
    |**Format données**|Spécifiez le format des données, le cas échéant. Par exemple, **MM/JJ/AAAA** si le type de données est **Date**. **Remarque :** pour l'exportation, spécifiez le format de données en fonction de [!INCLUDE[prod_short](includes/prod_short.md)]. Pour l'importation, spécifiez le format de données en fonction du .NET Framework. Pour plus d'informations, voir [Chaînes au format Date et Heure standard](https://go.microsoft.com/fwlink/?LinkID=323466).|  
...
3.  Sur le raccourci **Correspondance champ**, spécifiez la correspondance entre les colonnes et les champs dans [!INCLUDE[prod_short](includes/prod_short.md)] en renseignant les champs comme indiqué dans le tableau suivant.  
    ...
    |**Code du champ**|Spécifiez à quel champ correspond la colonne dans le champ **N° colonne** .<br /><br /> Vous pouvez uniquement choisir parmi des champs existant dans la table que vous avez spécifiée dans le champ **Table** du Raccourci **General**.|  
    |**En option**|Indiquez que la fiche sera ignorée si le champ est vide. **Remarque :** si vous n'activez pas cette case à cocher, une erreur d'exportation aura lieu si le champ est vide. **Remarque** : ce champ ne s'applique qu'aux exportations.|  
    |**Code de la table cible**|Uniquement visible lorsque la case à cocher **Utiliser comme table intermédiaire** est cochée.<br /><br /> Spécifiez la table à laquelle la valeur du champ **Titre colonne** est mappée lorsque vous utilisez une table intermédiaire pour l'importation des données.|  
    |**Libellé table cible**|Uniquement visible lorsque la case à cocher **Utiliser comme table intermédiaire** est cochée.<br /><br /> Spécifiez le nom de la table dans le champ **Code table cible**, qui est la table à laquelle la valeur du champ **Titre colonne** est mappée, lorsque vous utilisez une table intermédiaire pour l'importation des données.|  
    |**Code du champ cible**|Uniquement visible lorsque la case à cocher **Utiliser comme table intermédiaire** est cochée.<br /><br /> Spécifiez le champ dans la table cible auquel la valeur du champ **Titre colonne** est mappée lorsque vous utilisez une table intermédiaire pour l'importation des données.|  
