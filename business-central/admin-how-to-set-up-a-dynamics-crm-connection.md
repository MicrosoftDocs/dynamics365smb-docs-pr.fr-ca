...
## <a name="link-your-business-central-and-dataverse-environments"></a>Reliez votre Business Central et Dataverse environnements
...
Les compagnies [!INCLUDE [prod_short](includes/prod_short.md)] peuvent se connecter à un environnement [!INCLUDE [cds_long_md](includes/cds_long_md.md)] dont la devise de base est différente de leur devise locale.
...
La procédure suivante décrit comment configurer manuellement la connexion sur la page **Configuration de la connexion Dataverse**. C’est sur la page **Configuration de la connexion Dataverse** que vous gérez les paramètres d’intégration.
...
* Les pages de liste qui affichent les enregistrements synchronisés avec [!INCLUDE [cds_long_md](includes/cds_long_md.md)], telles que les pages Clients et Articles.  
    ...
    Lorsque vous démarrez le processus de couplage par correspondance à partir d’une liste de données de base, une tâche de couplage est programmée directement après que vous ayez spécifié les critères de couplage.  
* La page **Révision synchronisation complète Dataverse**.  
    ...
    Lorsque vous lancez le processus de couplage par correspondance à partir de la page **Révision synchronisation complète Dataverse**, une tâche de couplage est programmée après que vous ayez terminé la configuration.  
...
Les étapes suivantes supposent que vous utilisiez Microsoft Entra ID pour gérer les identités et les accès. Pour plus d’informations sur l’enregistrement d’une application dans Microsoft Entra ID, voir [Démarrage rapide : enregistrer une application avec la plateforme d’identité Microsoft](/azure/active-directory/develop/quickstart-register-app). 
