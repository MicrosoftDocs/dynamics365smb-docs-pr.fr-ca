...
Lorsque le client remplit le questionnaire, vous importez le fichier dans la nouvelle compagnie [!INCLUDE[d365fin](includes/d365fin_md.md)] du client. Votre client et vous-même devez valider les réponses au questionnaire avant de les appliquer à la compagnie.
...
Vous pouvez utiliser un questionnaire pour vous aider à déterminer la portée et les besoins de la configuration. Vous pouvez créer un questionnaire ou modifier un questionnaire existant en y ajoutant de nouvelles questions ou zones de questions.  
 ...
 Vous pouvez créer des questionnaires uniquement pour les tables de type configuration. Par exemple, vous pouvez utiliser cet outil pour entrer des informations dans les fenêtres suivantes :  
...
> [!NOTE]  
>  Pour afficher une liste complète des tables de configuration, choisissez l'icône ![Page ou rapport pour la recherche](media/ui-search/search_small.png "icône Page ou rapport pour la recherche"), entrez **Configuration**, puis sélectionnez le lien associé. Pour déterminer la portée de la migration des données d'enregistrements, utilisez la fonctionnalité de migration. Pour plus d'informations, voir [Migration des données client](admin-migrate-customer-data.md).  

1. Choisissez l'icône ![Page ou rapport pour la recherche](media/ui-search/search_small.png "icône Page ou rapport pour la recherche"), entrez **Questionnaire configuration**, puis sélectionnez le lien associé.  
...
1. Ouvrez la compagnie pour laquelle vous souhaitez remplir le questionnaire.
2. Choisissez l'icône ![Page ou rapport pour la recherche](media/ui-search/search_small.png "icône Page ou rapport pour la recherche"), entrez **Questionnaire configuration**, puis sélectionnez le lien associé.  
3. Sélectionnez le questionnaire pour la compagnie, puis choisissez l'action **Exporter vers Excel**, ou éventuellement l'action **Exporter vers fichier XML**.
...
La procédure suivante fournit un autre moyen d’accéder aux questionnaires de configuration. Elle est basée sur l’hypothèse que le colis configuration qui a été envoyé inclut des questionnaires.  

1. Après avoir importé un colis configuration, ouvrez la feuille configuration.  
...
3. Répondez aux questions, puis sélectionnez l'action **Appliquer les réponses**.  
...
Une tâche de validation courante consiste à vérifier qu'aucune chaîne de caractères n'a été entrée dans les champs de date. Ce processus de révision est nécessaire, car le format des réponses du questionnaire n'est pas validé automatiquement lorsque vous exécutez la fonction **Appliquer les réponses**.  
...
3. Pour chaque question, vérifiez que la valeur du champ **Réponse** correspond au format indiqué dans le champ **Option de réponse**. Par exemple, assurez-vous que l'adresse d'une compagnie est au format texte.  
...
Une fois la validation terminée, les données peuvent être affectées à la base de données.  
...
1. Choisissez l'icône ![Page ou rapport pour la recherche](media/ui-search/search_small.png "icône Page ou rapport pour la recherche"), entrez **Questionnaire configuration**, puis sélectionnez le lien associé. La fenêtre **Questionnaire config.** s'ouvre.  
...
- Pour appliquer l'ensemble du questionnaire, sélectionnez l'action **Appliquer les réponses**.  
- Pour appliquer les réponses pour une **Zone question** spécifique, sélectionnez l'action **Zones question**, sélectionnez une **Zone question** dans la liste, puis sélectionnez l'action **Appliquer les réponses**.  

### <a name="to-verify-that-answers-have-been-applied-successfully"></a>Pour vérifier que les réponses ont été correctement affectées  
1. Vérifiez les fenêtres de paramétrage des différents modules de [!INCLUDE[d365fin](includes/d365fin_md.md)]. Pour localiser la fenêtre, choisissez l'icône ![Page ou rapport pour la recherche](media/ui-search/search_small.png "icône Page ou rapport pour la recherche"), entrez le nom de la fenêtre de configuration, puis sélectionnez le lien associé.  
...
Vous avez maintenant effectué la configuration à l'aide des informations professionnelles et des règles du client.
