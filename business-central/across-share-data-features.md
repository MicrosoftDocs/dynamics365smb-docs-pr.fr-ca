...
Vous pouvez partager un ou plusieurs enregistrements dans une liste. Appuyez simplement sur le raccourci clavier Ctrl+C pour copier dans votre presse-papiers. Collez ensuite ce que vous avez copié dans une autre application en appuyant sur Ctrl+V. Par exemple, copier trois bons de commande et les coller dans un courriel affichera les commandes dans un tableau bien formaté.
