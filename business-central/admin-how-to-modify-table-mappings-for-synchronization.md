...
Lorsque vous créez la connexion entre les applications, [!INCLUDE[d365fin](includes/d365fin_md.md)] configure quelques mappages de table et de champ par défaut. Si vous le souhaitez, vous pouvez modifier les mappages de tables. Pour en savoir plus, consultez [Mappage d'entité standard pour la synchronisation](admin-synchronizing-business-central-and-sales.md#standard-entity-mapping-for-synchronization). Si vous avez modifié les mappages par défaut et souhaitez annuler vos modifications, choisissez **Utiliser la configuration de synchronisation par défaut** sur la page **Configuration de la connexion [!INCLUDE[d365fin](includes/cds_long_md.md)]**.
