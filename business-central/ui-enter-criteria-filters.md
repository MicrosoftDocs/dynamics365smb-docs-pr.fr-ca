...
- **Filtrer les totaux par**
  ...
  Certaines listes qui affichent des champs calculés, comme des montants et des quantités, incluent la section **Filtrer les totaux par** où vous pouvez ajuster les différentes dimensions qui ont une incidence sur les calculs. Par exemple, vous pouvez analyser rapidement votre plan comptable en filtrant les montants selon une période spécifique, ou vous pouvez afficher les totaux des documents de vente uniquement pour un entrepôt spécifique.

  Pour ajouter un filtre, sélectionnez **+ Filtre**, sélectionnez l'une des dimensions prédéfinies, puis ajoutez les critères de filtre dans la zone.
...
> [!NOTE]  
>   Vous ne pouvez pas utiliser `*` lors du filtrage des champs (énumération) d'option, tels que le champ **État** sur les documents de vente. Pour entrer un filtre pour ce type de champ, vous pouvez saisir la valeur numérique comme paramètre de filtre. Par exemple, dans le champ **État** sur un document de vente qui a les valeurs **Ouvert**, **Libéré**, **Approbation en attente** et **Paiement anticipé en attente**, utilisez les valeurs `0`, `1`, `2` et `3` pour filtrer ces options.
