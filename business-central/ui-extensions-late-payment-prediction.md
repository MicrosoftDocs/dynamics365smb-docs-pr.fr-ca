...
Vous pouvez également prévoir des retards de paiement par avance. Dans les fenêtres **Devis**, **Documents de vente**, et **Factures vente**, vous pouvez utiliser l'action **Prévoir le paiement** pour générer des prévisions pour le document vente que vous visualisez.
...
Cependant, nous offrons un moyen plus facile de créer et d'utiliser votre propre modèle de prévision. Vous pouvez partager les données de vos factures avec notre expérience prédictive dans Azure Machine Learning, et laisser notre expérience créer et expérimenter un modèle prédictif basé sur vos données. Pour partager vos données, dans la fenêtre **Configuration des prévisions de retard de paiement**, choisissez l'action **Créer mon modèle**. Ensuite, les prévisions seront basées sur votre modèle et vos données, pas les nôtres.  
