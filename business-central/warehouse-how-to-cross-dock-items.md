...
1. Si vous utilisez des zones, configurez au moins une zone de transbordement. Si vous utilisez le prélèvement et le rangement dirigés, configurez une zone de transbordement.  
    ...
    Un zone de transbordement a le champ **Zone de transbordement** sélectionné. Pour en savoir plus sur les zones, accédez à [Créer des zones](warehouse-how-to-create-individual-bins.md).  

    Si vous utilisez des zones, créez une zone pour vos zones de transbordement, puis sélectionnez le champ **Zone transbordement**. Si vous utilisez le rangement et le prélèvement suggérés, sélectionnez le type de zone avec **Prélever** sélectionné. Par exemple, vous pouvez utiliser *PICK* ou *PUTPICK*. Pour en savoir plus sur les zones et les types de zones, accédez à [Configurer les emplacements pour utiliser les zones](warehouse-how-to-set-up-locations-to-use-bins.md) et [Configurer les types de zones](warehouse-how-to-set-up-bin-types.md).  
...
4. Sur le raccourci **Entrepôt**, activez le bouton à bascule **Utiliser transbordement**, puis renseignez le champ **Date d’échéance transbordement** en y indiquant la période pendant laquelle le programme doit rechercher des opportunités de transbordement.
    ...
    L’option **Utiliser transbordement** n’est disponible que si vous avez sélectionné les champs **Réception requise**, **Livraison requise**, **Prélèvement requis** et **Rangement requis**.  
