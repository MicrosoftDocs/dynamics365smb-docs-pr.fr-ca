...
---
# <a name="set-up-payment-methods"></a>Configuration des modes de règlement
...
Toutefois, les notes de crédit pour les retours sont des exceptions, parce que le règlement s'effectue dans le sens inverse, de vous à votre client et de votre fournisseur à vous. Par conséquent, un mode de règlement par défaut n’est pas affecté aux notes de crédit. Il existe cependant une solution de contournement. Vous pouvez spécifier les conditions de paiement pour le client ou le fournisseur. Bien que le champ **Calculer escompte de paiement sur les notes de crédit** ne soit pas prévu pour cette solution de contournement, si vous cochez la case sur la page **Modalités de paiement**, un mode de règlement par défaut est ajouté lorsque vous créez une note de crédit. <br><br>  
