...
description: 'Cet article décrit les différentes manières de travailler avec la TVA à la fois manuellement et avec la configuration automatique, pour vous aider à respecter les réglementations spécifiques à chaque pays/région.'
author: brentholtorf
ms.author: bholtorf
...
### <a name="modifying-the-vat-date-in-posted-entries"></a>Modification de la date TVA dans les écritures reportées
...
Vous pouvez également ajuster les montants TVA dans les journaux généraux, vente et achat. Par exemple, vous devrez peut-être effectuer un ajustement si vous entrez une facture fournisseur dans votre journal et que le montant de TVA calculé par [!INCLUDE[prod_short](includes/prod_short.md)] et le montant de TVA figurant sur la facture du fournisseur diffèrent.  

### <a name="to-set-the-system-up-for-manual-vat-entry-in-general-journals"></a>Pour configurer le système pour la saisie manuelle de la TVA dans les journaux généraux
...
### <a name="to-create-a-new-invoice-for-the-import-authority-vendor-after-you-complete-the-setup"></a>Pour créer une facture pour le fournisseur de l’autorité d’importation, une fois la configuration terminée
...
4. Pour inclure des informations des lignes dans le document livraison sur le certificat, activez le bouton à bascule **Imprimer détails de ligne**.  
5. Pour que [!INCLUDE[prod_short](includes/prod_short.md)] crée des certificats pour les livraisons reportées qui n’en ont pas, activez le bouton à bascule **Créer des certificats d’approvisionnement s’ils n’ont pas encore été créés**. Lorsque vous activez le bouton à bascule, de nouveaux certificats sont créés pour toutes les livraisons reportées qui n’ont pas de certificats compris dans la plage sélectionnée.  
