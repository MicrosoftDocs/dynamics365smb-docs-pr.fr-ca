...
> [!TIP]
> Vous pouvez exporter les résultats du rapport dans un fichier Excel pour afficher l’ensemble de données complet, y compris toutes les colonnes, mais sans la présentation. Le fichier Excel peut vous aider à valider que le rapport renvoie les données attendues ou les problèmes de diagnostic. Pour plus d’informations, consultez [Utiliser les rapports - Envoyer vers Excel](ui-work-report.md#send-to-excel).
...
###  <a name="removing-label-and-data-fields-in-word-layouts"></a><a name="RemoveField"></a> Suppression des champs d'étiquette et de données dans les présentations Word
 ...
 ![Contrôle de contenu d’un champ dans une présentation rapport Word.](media/nav_wordreportlayouts_contentcontrol.png "NAV_WordReportLayouts_ContentControl")  
