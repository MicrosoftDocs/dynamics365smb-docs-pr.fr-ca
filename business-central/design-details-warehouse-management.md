...
|Avancé|Comme ci-dessus + activités de prélèvement/rangement dirigées|Prélèvement et rangement dirigés (les boutons à bascule dépendants sont activés automatiquement)|Obligatoire|Comme ci-dessus.|Comme ci-dessus.| Bon de production -> Journal prélèvement -> Prélèvement(s) entrepôt -> Journal consommation|
...
Les emplacements qui utilisent le rangement et le prélèvement dirigés peuvent utiliser des types de zone. Les types de zone contrôlent les activités que vous autorisez pour une zone. 

Les types d’emplacement disponibles sont les suivants :  
...
> [!NOTE]  
> Vous devez utiliser des mouvements pour déplacer les articles vers les zones RECEIVE et CQ. Utilisez aussi des mouvements pour déplacer des articles des zones SHIP et CQ.  
...
Dans les flux entrants, le code classe est uniquement sélectionné sur les lignes entrantes lorsque le code classe article ne correspond pas à la zone de réception par défaut. Si les zones correctes par défaut ne sont pas affectées, la quantité ne peut pas être reçue.  
