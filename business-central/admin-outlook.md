...
ms.reviewer: jswymer
---
# <a name="get-the-business-central-add-in-for-outlook"></a>Obtention du complément Business Central pour Outlook
...
- Votre compte possède au moins le rôle [Administrateur Exchange](/entra/identity/role-based-access-control/permissions-reference#exchange-administrator] Microsoft 365 
...
1. Sélectionnez **Déploiement centralisé du complément Outlook** pour démarrer le guide de configuration assistée.
1. Passez en revue la première page et sélectionnez **Suivant** pour ouvrir la page de téléchargement des compléments.
1. Dans la colonne **Déployer**, cochez la case des compléments que vous souhaitez déployer, puis sélectionnez **Télécharger et continuer**.

    Un fichier nommé *OutlookAddins.zip* est téléchargé sur votre appareil.

1. À ce stade, vous avez terminé le travail que vous devez effectuer dans Business Central, vous pouvez donc sélectionner **Terminé**.
...
1. Allez dans le dossier où OutlookAddins.zip a été téléchargé et extrayez les fichiers **Contact Insights.xml** et **Document View.xml** du .zip dans un dossier de votre choix.
...
1. Connectez-vous au Centre d’administration Microsoft 365, puis accédez à [Applications intégrées](https://go.microsoft.com/fwlink/?linkid=2163967).

1. Sélectionnez **Charger des applications personnalisées**.
1. Sur la page **Télécharger les applications à déployer** , définissez **Type d’application** sur **Complément Office**.
1. Sélectionnez **Télécharger le fichier manifeste (.xml) à partir de l’appareil** > **Choisir un fichier**.
1. Sélectionnez l’un des fichiers de complément que vous avez extraits précédemment, **Context Insights.xml** ou **Documents.xml**.
1. Suivez les instructions pour affecter des utilisateurs et déployer le complément.
1. Répétez les étapes 9 à 11 pour l’autre fichier de complément si vous le souhaitez.
