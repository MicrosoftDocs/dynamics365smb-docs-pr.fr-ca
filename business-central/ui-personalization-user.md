...
-->
> [!NOTE]
> Outre ce que les utilisateurs peuvent personnaliser, les administrateurs et les super utilisateurs peuvent remplacer les personnalisations des utilisateurs et définir les fonctionnalités accessibles dans toutes les compagnies ou des compagnies spécifiques. Pour plus d'informations, voir [Personnalisation de Business Central](ui-customizing-overview.md).
...
Un *groupe d'actions* désigne l'article qui se développe pour afficher d'autres actions ou groupes. Par exemple, dans l'illustration suivante, **Report** désigne un groupe d'actions.

Une *catégorie promue* désigne un groupe d'actions qui s'affiche entre les deux lignes verticales `|` de la barre d'actions. Les catégories incluent généralement les actions utilisées le plus fréquemment afin que vous puissiez les trouver rapidement. Par exemple, dans l'illustration suivante, **Libérer**, **Report**, **Facture** et **Naviguer** sont des catégories promues.
