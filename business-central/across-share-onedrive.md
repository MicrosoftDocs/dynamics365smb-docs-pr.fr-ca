...
4. Dans le dossier [!INCLUDE[prod_short](includes/prod_short.md)], il crée un dossier portant le même nom que la compagnie dans laquelle vous travaillez. Si vous travaillez dans plusieurs compagnies, [!INCLUDE[prod_short](includes/prod_short.md)] crée un dossier pour chaque compagnie où vous travaillez quand vous utilisez l’action **Ouvrir dans OneDrive** ou **Partager**. 
5. Place une copie du fichier que vous avez sélectionné dans le dossier au nom de la compagnie, puis ouvre le fichier. 
