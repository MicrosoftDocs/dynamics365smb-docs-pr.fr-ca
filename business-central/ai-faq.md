...
- Vous devez utiliser la version 22 ou ultérieure de Business Central Online (si vous êtes déjà client) ou une version d’évaluation.  <!--**22.0.54157.54311 (Preview - Copilot edition)**-->
   ...
   Pour vérifier la version, sélectionnez le point d’interrogation dans le coin supérieur droit, puis sélectionnez **Aide et support**. Sous **Dépannage**, recherchez la version de l’application. Pour plus d’informations sur la façon d’obtenir la bonne version préliminaire, accédez à [Commencer par une version préliminaire de Business Central](ai-preview-getstarted.md).
