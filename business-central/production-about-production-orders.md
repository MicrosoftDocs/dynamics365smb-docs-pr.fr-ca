...
Les bons de production sont les composantes centrales de la fonctionnalité de fabrication et contiennent les informations suivantes :  
...
Les bons de production sont créés à l’aide d’informations provenant des éléments suivants :  
...
- créés à partir de la fenêtre [Feuille planification](production-how-to-run-mps-and-mrp.md) ;  
- Créé à partir de la page [Planification document de vente](production-how-to-create-production-orders-from-sales-orders.md)  
...
- Le fait qu’un bon de production soit libéré ne signifie pas nécessairement que les matières sont prélevées ou que le projet a été déplacé physiquement vers la première opération qui y est associée.  
...
- Un bon de production terminé est généralement un ordre fabriqué.  
- L'achèvement du bon de production est une tâche importante dans l'évaluation du cycle de vie de l'article en cours de production. Après l’achèvement d’un bon de production, ajuster et rapprocher l’évaluation.  
...
La consommation de matières peut être traitée via le [journal consommation](production-how-to-post-consumption.md), mais peut également être enregistrée automatiquement par [!INCLUDE [prod_short](includes/prod_short.md)], processus de génération de rapport automatique (consommation). Les méthodes de génération de rapports Manuel, Avant et Arrière sont disponibles.
...
La production peut être traitée via le [journal de sortie](production-how-to-post-output-quantity.md), mais aussi enregistrée automatiquement. [!INCLUDE [prod_short](includes/prod_short.md)] copie la méthode de consommation de la fiche unité de production ou atelier vers l’itinéraire du bon de production lors de l’actualisation. Comme pour la consommation matière, les méthodes de génération de rapports Manuel, Avant, Arrière s’appliquent à la production.

La méthode manuelle utilise le journal de sortie pour spécifier le temps consommé et la quantité produite.  

En aval, cette méthode enregistre la production prévue (et le temps), qui est automatiquement enregistrée lors du lancement d’un bon de production. Les codes lien itinéraire n’interviennent pas comme facteurs dans la consommation en aval de la production.  
...
Le journal production a pour but de vous fournir une interface unique pour l’enregistrement de la consommation et de la production à partir d’un bon de production.  

Les journaux production présentent une vue simple et offrent les possibilités suivantes :  
...
- Mettre en relation des données d’opération réelles avec les estimations standard sur des lignes composante et itinéraire bon de production  
- Reporter et imprimer un aperçu des données d’opération enregistrées pour un bon de production  

Le journal production exécute un grand nombre des fonctions des journaux consommation et de sortie. Les dimensions, le suivi des articles et le contenu de la zone sont traités de la même manière.  

Toutefois, les journaux production diffèrent des journaux de sortie et consommation sur les plans suivants :  

- Elle est appelée directement depuis une ligne bon de production libérée et prédéfinie avec les données appropriées.  
