...
> [!NOTE]
> Une autre méthode permettant de définir les fonctionnalités auxquelles un utilisateur a accès consiste à définir le champ **Expérience** sur la page **Informations compagnie**. Pour plus d'informations, voir [Modifier les fonctionnalités affichées](ui-experiences.md).
>
...
Le nouveau groupe d'utilisateurs est ajouté à la page **Groupes d'utilisateurs**. Ajoutez ensuite des utilisateurs. Pour plus d'informations, reportez-vous à [Pour regrouper des utilisateurs dans des groupes d'utilisateurs](ui-define-granular-permissions.md#to-group-users-in-user-groups).  
