...
# <a name="assign-permissions-to-users-and-groups"></a>Affecter des autorisations aux utilisateurs et aux groupes
...
Cet article décrit comment vous pouvez définir, utiliser et affecter des autorisations dans [!INCLUDE [prod_short](includes/prod_short.md)] pour changer la configuration par défaut.  
...
> [!NOTE]
> Une façon plus vaste de définir les fonctionnalités auxquelles un utilisateur a accès consiste à définir le champ **Expérience** sur la page **Informations compagnie**. Pour plus d'informations, voir [Modifier les fonctionnalités affichées](ui-experiences.md).
>
...
6. Utilisez les champs **Type d’objet** et **Code d’objet** pour spécifier l’objet auquel vous donnez accès.

  > [!TIP]
  > Les nouvelles lignes affichent les valeurs par défaut. Par exemple, le champ **Type d’objet** contient **Données de table**, et le champ **Code d’objet** contient **0**. Les valeurs par défaut ne sont que des espaces réservés et ne sont pas utilisées. Vous devez choisir un type d’objet et un objet dans le champ **Code d’objet** avant de pouvoir créer une autre ligne.
...
L’autorisation indirecte vous permet d’utiliser un objet uniquement au travers d’un autre objet. Par exemple, un utilisateur peut être autorisé à exécuter le codeunit 80 (Ventes-Reporter). Le codeunit Ventes-Valider effectue de nombreuses tâches, parmi lesquelles modifier la table 37 (Ligne vente). Lorsque l’utilisateur reporte un document vente avec le codeunit Ventes-Reporter, [!INCLUDE[prod_short](includes/prod_short.md)] vérifie si l’utilisateur est autorisé à modifier la table Ligne vente. S’il n’est pas autorisé, le codeunit ne peut pas effectuer ses tâches et l’utilisateur reçoit un message d’erreur. S'il est autorisé à le faire, le codeunit s'exécute.

L’utilisateur n’a toutefois pas besoin d’avoir un accès total au tableau Ligne vente pour exécuter le codeunit. Si une autorisation indirecte a été accordée à l’utilisateur pour la table Ligne vente, le codeunit Ventes-Reporter s’exécute correctement. Lorsqu’une autorisation indirecte est accordée à un utilisateur, celui-ci peut uniquement modifier la table Ligne vente en exécutant le codeunit Ventes-Reporter ou un autre objet autorisé à modifier la table Ligne vente. L'utilisateur peut uniquement modifier la table Ligne vente lorsqu'il procède à partir des modules pris en charge. L’utilisateur ne peut pas exécuter cette fonctionnalité par inadvertance ou par malveillance en suivant d’autres méthodes.
...
Sur le volet **Résultat**, utilisez le champ **État d’inclusion** pour identifier les ensembles d’autorisations dans lesquels vous avez exclu des autorisations ou des ensembles d’autorisations. Si quelque chose a été exclu, l'état est défini sur **Partiel**.
...
## <a name="copy-a-permission-set"></a>Copier un ensemble d’autorisations
...
## <a name="create-or-modify-permissions-by-recording-your-actions"></a>Créer ou modifier des autorisations en enregistrant vos actions
...
## <a name="remove-obsolete-permissions-from-all-permission-sets"></a>Supprimer des autorisations obsolètes de tous les ensembles d’autorisations

Dans la page **Ensemble d'autorisations**, choisissez l'option **Supprimer les autorisations obsolètes**.

## <a name="set-up-time-constraints-for-users"></a>Configurer des contraintes de temps pour les utilisateurs
...
## <a name="control-access-to-specific-companies"></a>Contrôler l’accès à des compagnies spécifiques

Lorsque vous avez plusieurs compagnies dans Business Central, la gestion des autorisations entre les compagnies nécessite une attention particulière. Vous ne souhaitez peut-être pas que les utilisateurs disposent de droits d’accès identiques à toutes les compagnies. Au lieu de cela, vous devrez peut-être accorder des autorisations aux utilisateurs en fonction de leur affiliation à la compagnie. Pour prendre en charge ce scénario, lorsque vous attribuez des ensembles d’autorisations à des utilisateurs individuels ou à des groupes de sécurité, vous avez la possibilité de sélectionner une compagnie spécifique à laquelle l’ensemble d’autorisations s’applique. La compagnie n’est pas explicitement spécifiée dans l’ensemble d’autorisations, mais plutôt lorsque l’ensemble d’autorisations est attribué à l’utilisateur ou au groupe de sécurité.

Si vous ne spécifiez pas la compagnie lors de l’attribution d’un ensemble d’autorisations, l’ensemble d’autorisations s’applique alors à toutes les compagnies. Si vous souhaitez qu’un ensemble d’autorisations s’applique à plusieurs compagnies, mais pas à toutes les compagnies, ajoutez l’ensemble d’autorisations spécifiquement pour chaque compagnie séparément.
...
## <a name="manage-permissions-through-user-groups"></a>Gérer les autorisations via des groupes d’utilisateurs
...
> [!NOTE]
> Les membres du groupe d'utilisateurs ne sont pas copié vers le nouveau groupe d'utilisateurs. Vous devez les ajouter manuellement ensuite. Pour plus d’informations, voir la section [Pour ajouter des utilisateurs à un groupe d’utilisateurs](#to-add-users-to-a-user-group).
...
## <a name="assign-permission-sets-to-users"></a>Affecter des ensembles d’autorisations à des utilisateurs
...
1. Sur le raccourci **Ensembles d'autorisations utilisateur**, renseignez les champs, le cas échéant sur une nouvelle ligne. Pour plus d’informations, voir [Créer ou modifier des ensembles d’autorisations](ui-define-granular-permissions.md#create-a-permission-set).
   ...
   Définissez le champ **Compagnie** pour appliquer l’ensemble d’autorisations à une compagnie spécifique. Si vous souhaitez que l’ensemble d’autorisations s’applique à toutes les compagnies, laissez le champ **Compagnie** vide. [En savoir plus](#control-access-to-specific-companies).
...
1. Définissez le champ **Compagnie** pour appliquer l’ensembles d’autorisations à une compagnie spécifique uniquement. Si vous souhaitez que l’ensemble d’autorisations s’applique à toutes les compagnies, laissez le champ **Nom de la compagnie** vide. [En savoir plus](#control-access-to-specific-companies).
...
## <a name="get-an-overview-of-a-users-permissions"></a>Obtenir l’aperçu des autorisations d’un utilisateur
...
## <a name="view-permission-changes-telemetry"></a>Afficher de la télémétrie des modifications d’autorisation
...
[Affecter un code télémétrie aux utilisateurs](/dynamics365/business-central/dev-itpro/administration/telemetry-enable-application-insights#assign-a-telemetry-id-to-users)  
