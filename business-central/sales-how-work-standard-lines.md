...
4. Dans la fenêtre **Lignes vente récurrentes**, cliquez sur le bouton de recherche du champ **Code**, puis sélectionnez un ensemble de lignes vente standard.
    ...
    > [!NOTE]
    > Pour utiliser les lignes vente récurrentes définies avec le traitement en lot **Créer des factures vente récurrentes**, vous devez également renseigner les champs **Date début validité** et **Date fin validité** dans la fenêtre **Lignes vente récurrentes**. Pour plus d'informations, voir la section « Pour créer plusieurs factures vente basées sur des lignes vente standard ».
