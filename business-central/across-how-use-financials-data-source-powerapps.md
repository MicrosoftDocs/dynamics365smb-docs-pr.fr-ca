...
4. Power Apps affiche la liste des *Environnements et compagnies* disponibles à partir de [!INCLUDE [prodshort](includes/prodshort.md)]. Choisissez l'environnement et la compagnie contenant les données auxquelles vous souhaitez vous connecter. Une liste d’API est ensuite présentée. Sélectionnez l'**API**à laquelle vous voulez vous connecter.
