...
Après avoir rempli les informations et les lignes relatives au dépôt, vous devez les reporter. La publication met à jour les grands livres concernés, y compris le comptabilité, ainsi que les grands livres de la banque, des clients et des fournisseurs. Les dépôts affichés sont stockés sur la page **Dépôts bancaires reportés**, afin que vous puissiez vous y référer par la suite.
...
Les dépôts sont des encaissements, vous souhaiterez donc peut-être réutiliser votre série de numéros pour les journaux d’encaissements. Sinon, si vous devez faire la distinction entre les écritures journal des dépôts bancaires et des encaissements, utilisez une série de numéros différente.
...
6. Renseignez les champs restants selon vos besoins. [!INCLUDE [tooltip-inline-tip_md](../archive/SetupAndAdministration/includes/tooltip-inline-tip_md.md)]
    ...
    La date dans le champ  **Date de publication** et les dimensions dans les champs  **Code de service** et **Code de groupe de clients** sont affectées aux lignes que vous créez pour le dépôt bancaire. Si nécessaire, vous pouvez les modifier.
...
12. Si vous êtes prêt à reporter le dépôt bancaire, choisissez l’action **Reporter**.
    ...
    > [!NOTE]
    > Si le compte bancaire a des dimensions par défaut où le champ  **Publication de valeur** contient **Code obligatoire**, **Même code** ou **Aucun code**, vous devez comptabiliser le dépôt sous forme de somme forfaitaire. Si vous ne comptabilisez pas sous forme de somme forfaitaire, la comptabilisation peut échouer car les valeurs de dimension des comptes sur les lignes de dépôt bancaire enfreignent les règles de comptabilisation de valeur du compte bancaire.
